...
def compute_current(tile_series, f_max, k_max):
    ...
    # port = np.zeros(shape=(FOUR_TIME, CUT_NUM))
    # for ii in range(0, CUT_NUM):
    #     for jj in range(0, CUT_NUM):
    #         k = round(np.sqrt(ii * ii + jj * jj))
    #         if ii + HALF < back_f3.shape[1] and jj + HALF < back_f3.shape[2] and k < CUT_NUM:
    #             port[:, k] += back_f3[:, ii + HALF, jj + HALF] #  np.sign(ii) *
    # port[0, 0] = 0.
    # ax[1].imshow(port, origin="lower", aspect="auto", extent=[0, k_max, 0, f_max])
    # kkk = np.linspace(0, k_max, 100)
    # ax[1].plot(kkk, np.sqrt(9.81 * kkk) / 2 / np.pi, c="w")
    # ax[1].set_xlim(0, k_max)
    # ax[0].imshow(tile_series[0], origin="lower", aspect="auto")
    # plt.title(f"ux = {ux:.3f}, uy = {uy:.3f} м/с")
    # plt.show()
...
def plot_and_interpolate_velocity_field(coords, speeds, drone_vx, drone_vy, yaw_deg, out_dir, nx=100, ny=100):
    ...
    speeds_corr = (speeds_rot - np.array([drone_vx, drone_vy])) * 100

    vx, vy = speeds_corr[:, 0], speeds_corr[:, 1]

    # --- Интерполяция
    grid_x, grid_y = np.mgrid[
                     np.min(coords[:, 0]):np.max(coords[:, 0]):complex(nx),
                     np.min(coords[:, 1]):np.max(coords[:, 1]):complex(ny)
                     ]
    vx_i = griddata(coords, vx, (grid_x, grid_y), method='linear')
    vy_i = griddata(coords, vy, (grid_x, grid_y), method='linear')
    ...
    plt.quiver(coords[:, 0], coords[:, 1], vx, vy, color="k", scale=10, scale_units="xy", width=0.002)
    plt.xlabel("x (m)")
    plt.ylabel("y (m)")
